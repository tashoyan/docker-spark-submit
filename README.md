...
Run the container:
```
docker run -ti --rm --name spark-submit -e MAIN_CLASS="image.Generator0S" spark-submit
```
Run the container on a machine behind proxy:
```
docker run -ti --rm --name spark-submit -e MAIN_CLASS="image.Generator0S" -e https_proxy=http://my.proxy.com:8080 spark-submit
```
